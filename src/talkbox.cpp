...
{
	// reset state - matches VST suspend() behavior
	pos = K = 0;
	emphasis = 0.0f;
	FX = 0.0f;
	...
	// NOTE: window[] is NOT reset here - it's calculated once and reused
	// This matches the original VST behavior and prevents clicks on Apple Silicon
}
...
//	if(fs > 96000.0f) fs = 96000.0f;
	long n = BUF_MAX;
	long newO = (long)((0.0001f + 0.000004f * GetParameter(_QUAL)) * fs);
	...
	// Denormal protection for Apple Silicon
	const float denormal_offset = 1.0e-20f;
